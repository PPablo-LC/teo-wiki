...
async function emitPage(
  ...
  // Use an absolute base path so all resources resolve correctly
  // regardless of page depth. During local dev (--serve), the dev server
  // serves from root so we use "/" for the 404 page and "." for others
  // (matching what pathToRoot returns for root).
  const baseDir = ctx.argv.serve
